 ...
 * </table>
 * <p>
 * Shaders derived from {@link GVRShader} cannot have variants and they ignore
...
public class GVRShader
{
    ...
    protected static String sTransformUBOCode =
            ...
            + "#define u_view calcView()\n"
            + "#define u_view_i calcInverseView()\n"
            + "#define u_mvp calcMVP()\n"
            + "#ifdef HAS_MULTIVIEW\n"
            + "mat4 calcMVP()\n{\n"
            + "    float rendermask = (u_render_mask & (gl_ViewID_OVR + uint(1))) > uint(0) ? 1.0 : 0.0;\n"
            + "    mat4 mvp = u_matrices[u_matrix_offset];\n"
            + "    mvp[3][0] -= u_proj_offset * float(gl_ViewID_OVR);\n"
            + "    mvp = mvp * render_mask;\n"
            + "    return mvp;\n}\n"
            + "mat4 calcView()\n{\n"
            + "    mat4 view = u_matrices[1];\n"
            + "    view[3][0] -= u_proj_offset * float(gl_ViewID_OVR);\n"
            + "    return view;\n}\n"
            + "mat4 calcInverseView()\n{\n"
            + "    mat4 iview = u_matrices[2];\n"
            + "    iview[0][3] -= u_proj_offset * float(gl_ViewID_OVR);\n"
            + "    return iview;\n}\n"
            + "#else\n"
            + "mat4 calcMVP()\n{\n"
            + "    mat4 mvp = u_matrices[u_matrix_offset];\n"
            + "    mvp[3][0] -= u_proj_offset * float(u_right);\n"
            + "    return mvp;\n}\n"
            + "mat4 calcView()\n{\n"
            + "    mat4 view = u_matrices[1];\n"
            + "    view[3][0] -= u_proj_offset * float(u_right);\n"
            + "    return view;\n}\n"
            + "mat4 calcInverseView()\n{\n"
            + "    mat4 iview = u_matrices[2];\n"
            + "    iview[0][3] -= u_proj_offset * float(u_right);\n"
            + "    return iview;\n}\n"
            + "#endif\n";
