...
namespace gvr
{
    ...
    {
        mMatrixUniforms = createUniformBlock("uint u_matrix_offset; uint u_right; uint u_render_mask; mat4 u_matrices[2]", MATRIX_UBO_INDEX, "MatrixUniforms", 0);
    ...
    RenderTexture* GLRenderer::createRenderTexture(const RenderTextureInfo& renderTextureInfo)
    {
        ...
        {
            return new GLMultiviewRenderTexture(renderTextureInfo.fdboWidth,
                                                renderTextureInfo.fboHeight,
                                                renderTextureInfo.multisamples, 2,
                                                renderTextureInfo.fboId, renderTextureInfo.texId);
        }
        return new GLNonMultiviewRenderTexture(renderTextureInfo.fdboWidth,
                                               renderTextureInfo.fboHeight,
                                                   ...
                                                   const TextureParameters *texparams, int number_views, bool monoscopic)
    {
        if (number_views == 1)
        {
            return new GLNonMultiviewRenderTexture(width, height, sample_count, jcolor_format,
                                                   jdepth_format,
